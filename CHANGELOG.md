...
**アップロード画面にExcelプレビュー＆手動カラムマッピングUIを新設し、ユーザーが列の対応を視覚的に確認・修正できるようになりました。薬価マスター同期をVercel Cron Jobで日次自動化。管理画面・提案フロー・アップロード画面のナビゲーションを統一し、画面遷移の一貫性を確保。MyRequestsPageを5モジュールに分割してコンポーネント肥大化を解消。さらに管理画面にマッチング実験ページ・エラーコードページ・アップロード品質ページを新設し、運用可視性を大幅に強化しました。**

---

#### 1. Excelプレビュー＆手動カラムマッピングUI

**今まで**: CSVやExcelファイルをアップロードする際、システムが自動でカラムを推定していましたが、推定が外れた場合に修正する手段がありませんでした。ユーザーはアップロード後にエラーが出て初めてマッピングの問題に気づき、ファイルを修正して再アップロードする必要がありました。

**今後**: アップロード前にExcelファイルのプレビューが表示され、各列とシステムフィールドの対応を視覚的に確認・手動で修正できます。`ColumnMappingForm` コンポーネントにより、ドロップダウンで列の割り当てを変更でき、プレビューテーブルで実データを確認しながらマッピングを調整できます。
...
#### 2. 薬価マスター日次Cron同期（Vercel Cron Job）

**今まで**: 厚生労働省の薬価基準データの更新は、手動で管理者がOpenClaw同期を実行する必要がありました。更新漏れがあると、古い薬価データでマッチング・提案が行われるリスクがありました。

**今後**: `/api/internal/drug-master-sync` エンドポイントをVercel Cron Jobとして登録し、毎日自動で薬価マスターを同期します。`vercel.json` にCronスケジュールを追加し、`CRON_SECRET` による認証でセキュリティも確保。同期結果はログに記録され、管理画面から確認できます。

```json
// vercel.json
{
  "crons": [{
    "path": "/api/internal/drug-master-sync",
    "schedule": "0 3 * * *"
  }]
}
```

#### 3. ナビゲーション統一（管理画面・提案フロー・アップロード）

**今まで**: 管理画面の各ページ、提案詳細画面、アップロード画面で、ナビゲーション（パンくずリスト・前後遷移・関連ページへのリンク）の実装がバラバラでした。管理画面は各ページに独自のナビリンクがハードコードされ、提案フローには前後の提案への遷移がなく、画面間の移動に一貫性がありませんでした。

**今後**: `AdminNavigationLinks` と `ProposalNavigationLinks` の共通コンポーネントを新設し、全画面で統一されたナビゲーションを提供。`MobileBottomNav` も拡張され、管理画面・提案フロー・アップロード画面すべてで一貫した画面遷移が可能になりました。`Header` と `Sidebar` も更新され、現在位置のハイライトとコンテキストに応じたリンク表示が統一されています。

#### 4. MyRequestsPageモジュール分割

**今まで**: `MyRequestsPage.tsx` が1ファイルで678行以上あり、リクエスト一覧表示・新規リクエスト作成・スレッド表示・ステータスフィルタリング・ヘルパー関数がすべて混在していました。機能追加やバグ修正時にファイル全体を読む必要があり、変更の影響範囲が把握しにくい状態でした。

**今後**: `my-requests/` ディレクトリに5つのモジュールとして分割しました。各モジュールが独立したテストを持ち、変更時の影響範囲が明確になりました。

```
client/src/pages/my-requests/
├── NewRequestSection.tsx    # 新規リクエスト作成フォーム
├── RequestListPane.tsx      # リクエスト一覧パネル
├── RequestThreadPane.tsx    # スレッド詳細表示
├── helpers.ts               # ステータス判定・フィルタリングロジック
└── types.ts                 # 型定義
```

#### 5. 管理画面の新規ページ追加

**今まで**: マッチング実験の実行・比較、エラーコードの管理、アップロード品質の可視化は、管理画面から直接行う手段がありませんでした。

**今後**: 3つの新規管理ページを追加しました：

- **AdminMatchingExperimentsPage**: マッチングアルゴリズムのA/B実験を作成・実行・比較。パラメータを変更した実験結果を並べて確認できます。
- **AdminErrorCodesPage**: システムエラーコードの一覧・検索・詳細表示。障害対応時にエラーコードから原因を素早く特定できます。
- **UploadQualityPage**: アップロードデータの品質メトリクス（欠損率、重複率、マッピング成功率）をダッシュボード形式で表示。

#### 6. useAdminPharmacyEdit フック改善

**今まで**: 薬局編集画面のフック `useAdminPharmacyEdit` は単一の巨大な状態オブジェクトで全フィールドを管理しており、型定義が不明確で、バリデーションロジックがコンポーネント側に散在していました。

**今後**: 型定義を `useAdminPharmacyEdit.types.ts` に分離し、フック内部の状態管理を整理。バリデーションロジックをフック内に集約し、コンポーネント側はフックの返り値を使うだけのシンプルな構造になりました。

#### 7. テスト大幅追加

**今まで**: 管理画面の多くのページ、ナビゲーションコンポーネント、新規追加されたヘルパー関数にはテストがありませんでした。

**今後**: 30以上のテストファイルを新規追加・拡充。特に以下が強化されました：

- 管理画面の全新規ページ（AdminMatchingExperiments、AdminErrorCodes、AdminNotifications等）
- MyRequestsPage分割後の各モジュール（helpers.test.ts、my-requests-page.test.tsx）
- ナビゲーションコンポーネント（Header、Sidebar、MobileBottomNav）
- Cron同期エンドポイント（internal-drug-master-sync-route.test.ts）
- Excelアップロード E2Eフロー（upload-excel-e2e-flow.test.ts）

#### 8. 開発ツール整理（dev/ ディレクトリ）

**今まで**: E2Eテスト（`e2e/`）、Remotion動画生成（`remotion/`）、テストデータ（`test-data/`）がプロジェクトルート直下に散在し、本番コードとの区別がつきにくい状態でした。

**今後**: 開発専用ファイルを `dev/` ディレクトリ配下に集約しました。

```
dev/
├── e2e/           # Playwright E2Eテスト
├── remotion/      # 動画生成
├── test-data/     # テスト用Excelファイル
└── README.md      # 開発ツールの説明
```
...
**提案の自動期限切れ・テンプレート機能を追加し、提案ワークフローをより実用的に。SSE 再接続の安定性向上、競合エラーのハンドリング改善、Playwright による E2E テスト基盤の導入など、運用品質と開発基盤を同時に強化しました。**

---

#### 1. 提案の自動期限切れ（Cron ジョブ）

**今まで**: 放置された提案がいつまでもアクティブなまま残り、マッチング候補を不必要にブロックしていました。手動でキャンセルする必要があり、運用負荷がかかっていました。

**今後**: 毎時実行の Cron ジョブが、一定期間応答のない提案を自動的に期限切れにします。マッチング候補が解放され、新しい提案に利用可能になります。

```
vercel.json:
{
  "path": "/api/internal/proposals/expire-stale",
  "schedule": "0 * * * *"
}
```
...
**今まで**: 提案を作成する際、毎回ゼロからメッセージや条件を入力する必要がありました。よく使うパターンを再利用する仕組みがありませんでした。

**今後**: 提案テンプレートの CRUD API を追加。よく使う提案パターンを保存・再利用でき、提案作成の効率が向上します。

#### 3. SSE 再接続のバックオフ制御

**今まで**: SSE（Server-Sent Events）接続が切断されると、ブラウザがデフォルトの再接続を即座に繰り返し、サーバーに不要な負荷をかけていました。ネットワーク不安定時に大量のエラーリクエストが発生する可能性がありました。

**今後**: 連続エラーを追跡し、5回連続エラーで接続を一旦閉じて30秒後に再接続する仕組みを導入。サーバー負荷を抑えつつ、復帰時は自動的にデータを再取得します。

#### 4. 競合エラーのハンドリング改善

**今まで**: 提案詳細画面で他のユーザーが先に操作した場合、「操作に失敗しました」という汎用エラーが表示されるだけで、状況が分かりませんでした。

**今後**: 409 Conflict エラーを検出すると「他のユーザーが先に操作しました。画面を更新します」と表示し、自動的に最新データを再取得します。

#### 5. アップロード待機 UX の改善

**今まで**: Excel アップロードの処理がタイムアウトすると「待機時間が上限を超えました」というエラーメッセージが表示され、処理が失敗したように見えていました。

**今後**: 「バックグラウンドで処理を継続中です。完了時に通知でお知らせします。」と表示し、ユーザーに安心感を提供します。

#### 6. E2E テスト基盤（Playwright）

**今まで**: エンドツーエンドのテスト基盤がなく、画面を跨いだフロー（アップロード→マッチング→提案→承認）の自動検証ができませんでした。

**今後**: Playwright を導入し、E2E テストの基盤を構築。提案フローのテストスケルトンを用意し、データシード実装後に即座にテストを拡充できる状態にしました。

#### 7. インフラ改善

- Node.js バージョンを 24.x に統一（CI・engines・.nvmrc）
- Vitest を `forks` プールに切り替え、テスト分離性を向上
- `.gitignore` に Playwright 関連ディレクトリを追加

## [0.0.20] - 2026-03-25

### テーマ: ヘッダー通知ドロップダウン + DDS リモートエージェント + モバイルレイアウト改善 + 管理画面リアルタイム強化

**ヘッダーの通知バッジをクリック可能なドロップダウンに進化させ、最新の通知一覧から各イベントの詳細ページへ直接ジャンプできるようになりました。OpenClaw DDS リモートエージェント連携、モバイル全画面のレイアウト最適化、管理画面のリアルタイムワークフロー改善など、運用品質を全方位で向上させています。**

---

#### 1. ヘッダー通知ドロップダウン

**今まで**: ヘッダー右上の通知バッジ（赤い数字）をクリックすると、ダッシュボードのトップに戻るだけでした。未読通知の内容を確認するにはタイムラインまでスクロールする必要があり、「何の通知が来たのか」を素早く確認する手段がありませんでした。

**今後**: 通知ベルアイコンをクリックすると、最新8件の通知がポップオーバーで即座に表示されます。各通知には提案詳細・アラート・マッチング結果など、該当ページへの直リンクが付いており、ワンクリックで遷移できます。未読イベントは青いボーダーと背景で一目で識別可能。「すべて既読」ボタンで一括既読化も対応しています。

```
表示内容:
- 🔔 ベルアイコン + 未読数バッジ
- 通知アイテム: アイコン / タイトル / 本文 / 経過時間 / 「詳細を見る →」リンク
- 「すべて既読」ボタン
- 「すべての通知を見る」→ ダッシュボードへ
```

デスクトップ・モバイル両対応。アップデートポップオーバーとの排他制御により、同時に1つだけ開く設計です。

#### 2. OpenClaw DDS リモートエージェント連携

**今まで**: OpenClaw DDS（Drug Distribution System）との連携は設計段階にとどまり、リモートでのエージェント実行やフロー制御の仕組みがありませんでした。

**今後**: DDS リモートエージェントランナーを実装し、OpenClaw からのリモートコマンド実行に対応しました。接続の安定化・リトライ・フロー制御を含む堅牢な連携基盤を構築しています。

- エージェントランナー統合 (`enable DDS remote agent runner integration`)
- 接続の安定化とフロー制御 (`stabilize OpenClaw DDS remote agent flow`)
- プレビュー環境でのゲート制御とフロー整合 (`stabilize preview gate and align openclaw dds flow`)

#### 3. モバイルレイアウト全面改善

**今まで**: スマートフォンやタブレットで一部の画面レイアウトが崩れたり、テーブルがはみ出したり、ボタンが小さすぎてタップしづらい箇所がありました。

**今後**: 全ページのモバイルレイアウトを見直し、レスポンシブ対応を強化しました。カード・テーブル・フォーム・KPIタイルなど、主要UIコンポーネントのモバイル表示を最適化しています。

#### 4. 管理画面リアルタイムワークフロー改善

**今まで**: 管理画面でのリアルタイム通知やメッセージ管理のワークフローに不足がありました。管理者が即座にアクションを取りづらい場面がありました。

**今後**: 管理画面のリアルタイムワークフローを改善し、通知配信・メッセージング・ユーザー対応のフローを強化しました。Admin UX のポリッシュにより、操作の一貫性と効率が向上しています。

#### 5. インクリメンタル検索の精度向上

**今まで**: 医薬品名やYJコードの検索で、入力途中の候補表示が期待通りの並び順にならないことがありました。部分一致の精度が低く、目的の医薬品を見つけるまでに余分な操作が必要でした。

**今後**: インクリメンタル検索のスコアリングアルゴリズムを見直し、前方一致・完全一致を優先する重み付けに改善しました。入力に応じてより関連性の高い候補が上位に表示されます。

#### 6. DB テストスキーマの安定化

**今まで**: テスト用のPGliteスキーマとプロダクションスキーマの間にドリフト（差分）が発生し、テストが偽陽性・偽陰性を出すことがありました。

**今後**: テストDBスキーマの生成プロセスを修正し、プロダクションスキーマとの同期を保証するようにしました。スキーマドリフトによるテスト不安定が解消されています。

#### 7. SPA フォールバックリライト追加

**今まで**: Vercel にデプロイされたSPAで、直接URLアクセスやブラウザのリロード時に404エラーが発生するケースがありました。

**今後**: `server/vercel.json` にSPAフォールバックリライトルールを追加し、全ルートが正しく `index.html` にフォールバックするようになりました。

---

## [0.0.19] - 2026-03-24

### テーマ: リアルタイム通知 + Stripe決済 + OpenClaw連携強化 + オンボーディング刷新 + 管理画面拡充

**薬局間のやりとりをリアルタイムに通知する SSE 基盤を導入し、提案の進捗や新着メッセージを即座に把握できるようになりました。Stripe連携によるサブスクリプション決済、OpenClawとの双方向リアルタイム連携、ステップ形式のオンボーディングウィザード、メッセージング機能、ブックマーク機能など、ユーザー体験を大幅に拡充しています。管理画面にも一括操作・A/Bテスト・レート制限管理・OpenClaw接続管理を追加し、運用効率を向上させました。**

---

#### 1. リアルタイム通知（SSE + Redis Pub/Sub）

**今まで**: 新しい提案やメッセージが届いても、画面をリロードしない限り気づけませんでした。ブラウザを開いたままにしていても「新着あり」の表示が更新されず、対応が遅れることがありました。

**今後**: Server-Sent Events（SSE）による常時接続で、提案の更新・新着メッセージ・アラートなどがリアルタイムにブラウザへプッシュされます。Redis Pub/Sub を使ったサーバー間のイベント配信に対応しており、複数のサーバーレスインスタンスに接続しているユーザーにも確実に通知が届きます。ヘッダーの通知バッジがリアルタイムで更新され、重要な変更を見逃しません。

```
新規サービス:
- realtime-service.ts — SSE 接続管理・イベント配信
- redis-pubsub-service.ts — Redis Pub/Sub による分散イベント
- sse.ts / realtime.ts — SSE エンドポイント
```

#### 2. Stripe サブスクリプション決済

**今まで**: サービスの利用プランや決済の仕組みがなく、全ユーザーが同じ機能を利用していました。将来的なプラン分けや収益化の基盤が整っていませんでした。

**今後**: Stripe Checkout を使ったサブスクリプション決済フローを実装しました。チェックアウトセッションの作成、Webhook によるイベント受信（支払い完了・サブスクリプション更新・キャンセル）、DBへの課金状態の永続化まで一通り対応しています。成功/キャンセル時のリダイレクトページも用意されています。

```
新規ファイル:
- stripe-service.ts — Stripe API 連携
- subscription-service.ts — サブスクリプション状態管理
- stripe-webhook.ts — Webhook 受信・検証
- subscriptions.ts — チェックアウト・ポータルAPI
- SubscriptionSuccessPage.tsx / SubscriptionCancelPage.tsx
```

#### 3. OpenClaw リアルタイム連携強化

**今まで**: OpenClaw（AI薬剤師エージェント）との連携は、手動でコマンドを送信し結果を待つ一方通行のフローでした。接続状態の監視や、失敗時の自動リトライの仕組みがありませんでした。

**今後**: OpenClaw との双方向リアルタイム連携を実現しました。

- **接続管理** (`openclaw-connect.ts`): OpenClaw との WebSocket/SSE 接続の確立・再接続・ハートビートを管理。接続状態は管理画面からリアルタイムで確認可能
- **リトライサービス** (`openclaw-retry-service.ts`): 失敗した OpenClaw リクエストを指数バックオフで自動リトライ。リトライ履歴とステータスを管理画面から一覧確認可能
- **リクエストイベントサービス** (`openclaw-request-event-service.ts`): OpenClaw とのやりとりをイベントとして記録し、タイムライン表示に対応
- **ヘルスKPI** (`openclaw-health-service.ts`): 応答時間・成功率・エラー率を計測し、管理ダッシュボードに表示
- **スレッド管理** (`openclaw-thread-service.ts`): 会話スレッドの作成・管理・履歴保持

**管理画面の強化**:
- OpenClaw 接続管理ページ（接続/切断/ステータス表示）
- リトライキュー管理（一覧/手動リトライ/パージ）
- ヘルスKPIダッシュボード（応答時間・成功率のグラフ）

#### 4. オンボーディングウィザードの刷新

**今まで**: 新規登録後の初期設定は1つの長いフォームページで構成されていました。入力項目が多く、何から始めればよいか分かりにくいため、途中で離脱するケースがありました。

**今後**: 3ステップのウィザード形式に刷新しました。各ステップにプログレスバーが表示され、進捗が一目で分かります。ステップ間の遷移はアニメーション付きで、入力内容はステップをまたいで保持されます。
...
```
新規コンポーネント:
- OnboardingProgressBar — 進捗表示バー
- OnboardingStep1 / Step2 / Step3 — 各ステップの入力フォーム
```

#### 5. メッセージング機能

**今まで**: 薬局間のコミュニケーションは提案へのコメント（交換提案に紐づくスレッド）に限定されていました。提案に関係しない一般的な連絡や質問を送る手段がなく、電話やFAXに頼る必要がありました。

**今後**: 薬局間のダイレクトメッセージ機能を追加しました。未読/既読の管理、メッセージ一覧ページ、相手薬局のプロフィール表示に対応。コメントの未読管理も強化され、既読にしたコメントは次回表示時にバッジが消えます。

```
新規ファイル:
- messaging-service.ts — メッセージ送受信・既読管理
- messages.ts — メッセージAPI エンドポイント
- MessagesPage.tsx — メッセージ一覧・送受信画面
```

#### 6. ブックマーク機能

**今まで**: 気になるマッチング候補や薬剤を後で確認したい場合、URLをメモしたりスクリーンショットを撮るしかありませんでした。

**今後**: マッチング候補をワンクリックでブックマークし、専用のブックマーク一覧ページから再確認できます。ブックマーク数はヘッダーのバッジに表示されます。

```
新規ファイル:
- match-bookmarks.ts — ブックマークAPI
- BookmarksPage.tsx — ブックマーク一覧ページ
```

#### 7. 要望（リクエスト）ページの統合

**今まで**: 要望の投稿・一覧ページがサイドバーの深い階層にあり、存在に気づきにくい状態でした。また管理者向けのメニューもサイドバーの下部に配置されていました。

**今後**: 要望ページ（MyRequestsPage）をヘッダーのナビゲーションに統合し、全ユーザーが容易にアクセスできるようになりました。管理者メニューもヘッダー上部に移動し、サイドバーのメニュー構成をシンプル化しました。

#### 8. 検索の関連度ランキング

**今まで**: 薬剤マスター検索は単純な前方一致/部分一致で、検索結果の並び順がデータの登録順でした。「ロキソニン」で検索すると、ロキソニンテープやロキソニンゲルが先に表示され、最も一般的なロキソニン錠がリストの後方に来ることがありました。

**今後**: 全検索エンドポイントに関連度スコアリングを導入しました。完全一致 > 前方一致 > 部分一致 の優先度で結果がランキングされ、最も関連性の高い結果が先頭に表示されます。キーボード操作でのスクロール追従（scroll-into-view）にも対応しています。

#### 9. 管理画面の大幅拡充

**今まで**: 管理画面は基本的な薬局管理と統計表示に限定されていました。マッチングルールの実験的な変更や、レート制限の状況確認、薬局の一括操作などは、データベースを直接操作する必要がありました。

**今後**: 以下の管理機能を追加しました：

- **一括操作** (`admin-bulk-actions.ts`): 複数薬局の一括アクティベーション/非アクティベーション。安全のため確認ダイアログとバッチサイズ制限付き
- **A/Bテストフレームワーク** (`matching-experiment-service.ts`): マッチングアルゴリズムの変更を A/B テストとして実行し、効果を計測可能
- **レート制限管理** (`admin-rate-limits.ts`, `AdminRateLimitsPage.tsx`): 各エンドポイントのレート制限状況をリアルタイムで確認・調整
- **アップロード品質スコア** (`upload-quality.ts`): アップロードされたCSVの品質スコアを表示（マッチング率、カバー率等）
- **日次統計バッチ** (`daily-statistics-service.ts`): マッチング成功率・在庫回転率などを日次で集計・保存
- **トレンドチャート** (`TrendChart.tsx`, `RiskBucketBarChart.tsx`): 統計ページにトレンド折れ線グラフとリスクバケット棒グラフを追加
- **マッチングフィルター** (`MatchingFilters.tsx`): マッチング画面にフィルターパネルを追加（価格帯・期限・距離）

#### 10. デッドストック自動アーカイブ・統計バッチ

**今まで**: 期限切れのデッドストックがリストに残り続け、一覧が肥大化していました。また統計データは表示時にその場で集計していたため、データ量が増えるとダッシュボードの表示が遅くなっていました。

**今後**: 内部APIによるデッドストックの自動アーカイブと、日次統計の事前集計バッチを導入しました。アーカイブは期限切れから一定期間経過したアイテムを自動で非表示にし、日次統計はCron Jobで毎日深夜に集計してキャッシュします。

#### 11. 認証のセキュリティ強化

**今まで**: パスワード認証ルートに `LEGACY_PASSWORD_AUTH_ENABLED` ガードが一部欠落しており、WorkOS移行後もパスワードログインが意図せず有効になるリスクがありました。また、一部のAPIエンドポイントで入力バリデーションやエラーハンドリングが不十分でした。

**今後**: 全パスワード認証ルートに `LEGACY_PASSWORD_AUTH_ENABLED` ガードを復元し、WorkOS移行完了後に確実にパスワードログインを無効化できるようにしました。認可チェック・入力バリデーション・エラーハンドリングも見直し、セキュリティレビューの指摘事項をすべて対応しています。

#### 12. DDS エージェントサービス

**今まで**: デッドストックの分析・提案生成は手動の判定ロジックに依存しており、薬剤の特性や市場動向を考慮した高度な判定ができませんでした。

**今後**: `dds-agent-service.ts` を新規追加し、デッドストック分析のAIエージェント基盤を整備しました。在庫分析・リスク評価・提案生成の各ステップをサービスとして構造化しています。
