...
type OpenClawTaskKind = 'user_report' | 'incident_investigation' | 'verification_review';
interface OpenClawTaskConversationMessage {
  id?: number;
  authorType: string;
  messageType: string;
  body: string;
...
interface OpenClawTaskCallbacks {
  reportUrl: string;
  callbackUrl: string;
  commandsUrl: string;
...
interface OpenClawTaskEnvelope {
...
function buildGatewayCliMessage(task: OpenClawTaskEnvelope): string {
  return [
