...
router.get('/alerts', async (req: AuthRequest, res: Response) => {
  try {
    ...
    handleAdminError(err, 'Admin alerts list error', 'アラート一覧の取得に失敗しました', res);
  }
});

router.post('/alerts/bulk-resolve', adminWriteLimiter, async (req: AuthRequest, res: Response) => {
  try {
    const { ids } = req.body;
    ...
    handleAdminError(err, 'Admin alerts bulk resolve error', 'アラートの一括解決に失敗しました', res);
  }
});

router.get('/alerts/trends', async (_req: AuthRequest, res: Response) => {
  try {
    const trends = await getAlertTrends();
    res.json({ data: trends });
  } catch (err) {
    handleAdminError(err, 'Admin alerts trends error', 'アラート傾向の取得に失敗しました', res);
  }
});
